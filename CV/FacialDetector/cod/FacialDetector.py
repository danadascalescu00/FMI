...
class FacialDetector:
    ...
    def get_positive_descriptors(self):
        ...
        print("Dupa ce am extras toti descriptorii pentru imaginile pozitive obtinem un array de dimensiuni: ", positive_descriptors.shape)
    ...
    def get_negative_descriptors(self):
        ...
        print("Dupa ce am extras toti descriptorii pentru imaginile negative obtinem un array de dimensiuni: ", negative_descriptors.shape)
    ...
    def image_pyramid(self, image, original_size, min_size):
        ...
        while True:
            new_height, new_width = int(image.shape[0] - image.shape[0] * self.params.scaling_ratio), int(image.shape[1] - image.shape[1] * self.params.scaling_ratio)
    ...
    def run(self, return_descriptors=False):
        ...
        for i in range(num_test_images):
            ...
            for img_resised in self.image_pyramid(image=img, original_size=(L,C), min_size=self.params.dim_window):
                ...
                hog_descriptor = hog(image=img_resised, pixels_per_cell=(self.params.dim_hog_cell,self.params.dim_hog_cell), 
            ...
            if current_image_detections is not None:
                if self.params.use_hard_mining and return_descriptors:
                    current_image_detections, current_image_scores, current_image_descriptors = self.non_maximum_suppression(current_image_detections, current_image_scores, np.array([L,C]), current_image_descriptors)
                else:
                    current_image_detections, current_image_scores = self.non_maximum_suppression(current_image_detections, current_image_scores, np.array([L,C]))
            ...
            if current_image_detections is not None:
                ...
                file_names = np.concatenate((file_names, np.array([current_file_name] * current_image_scores.shape[0])), axis=None)
                if descriptors_to_return is not None:
